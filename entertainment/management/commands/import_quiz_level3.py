"""
Management command to import Level 3 (Fill the Blank) quiz questions
Usage: python manage.py import_quiz_level3
"""
...
class Command(BaseCommand):
    help = 'Import Level 3 Fill the Blank quiz questions'

    def handle(self, *args, **options):
        # JSON data from file (cleaned)
        questions_data = {
            "level_3": [
        ...
        # Create or get Level 3 Quiz
        quiz, created = Quiz.objects.get_or_create(
            slug='fill-the-blank-challenging',
            defaults={
                'category': category,
                'title': 'Fill the Blank - Challenging',
                'description': 'Level 3: Complete the sentence with the missing word',
                'difficulty_level': 3,
                'is_active': True,
                'is_daily': False,
                'max_questions': 10,
        ...
        for index, q_data in enumerate(questions_data['level_3']):
