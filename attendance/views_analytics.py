...
class RosterAnalyticsViewSet(ViewSet):
    ...
    def staff_summary(self, request):
        ...
        rows = RA.staff_totals(self._hotel(request), d['start'], d['end'], d.get('department'))
        payload = [{
            'staff_id': r['staff_id'],
    ...
    def department_summary(self, request):
        ...
        rows = RA.department_totals(self._hotel(request), d['start'], d['end'])
        payload = [{
            'department': r['department'],
            'total_rostered_hours': float(r['total_rostered_hours'] or 0),
            'shifts_count': r['shifts_count'],
    ...
    def kpis(self, request):
        ...
        k = RA.kpis(self._hotel(request), d['start'], d['end'])
        data = {
    ...
    def daily_totals(self, request):
        ...
        rows = RA.daily_totals(self._hotel(request), d['start'], d['end'], d.get('department'))
        payload = [{
            'date': r['shift_date'],
            'total_rostered_hours': float(r['total_rostered_hours'] or 0),
            'shifts_count': r['shifts_count'],
    ...
    def daily_by_department(self, request):
        params = PeriodQuerySerializer(data=request.query_params)
        params.is_valid(raise_exception=True)
        d = params.validated_data

        rows = RA.daily_by_department(self._hotel(request), d['start'], d['end'])
        payload = [{
            'date': r['shift_date'],
            'department': r['department'],
            'total_rostered_hours': float(r['total_rostered_hours'] or 0),
    ...
    def daily_by_staff(self, request):
        params = PeriodQuerySerializer(data=request.query_params)
        params.is_valid(raise_exception=True)
        d = params.validated_data

        rows = RA.daily_by_staff(self._hotel(request), d['start'], d['end'], d.get('department'))
        payload = [{
            'date': r['shift_date'],
            'staff_id': r['staff_id'],
            'first_name': r['staff__first_name'],
    ...
    def weekly_totals(self, request):
        ...
        rows = RA.weekly_totals(self._hotel(request), d['start'], d['end'], d.get('department'))
        payload = [{
            'year': r['year'],
            'week': r['week'],
            'total_rostered_hours': float(r['total_rostered_hours'] or 0),
    ...
    def weekly_by_department(self, request):
        params = PeriodQuerySerializer(data=request.query_params)
        params.is_valid(raise_exception=True)
        d = params.validated_data

        rows = RA.weekly_by_department(self._hotel(request), d['start'], d['end'])
        payload = [{
            'year': r['year'],
            'week': r['week'],
            'department': r['department'],
    ...
    def weekly_by_staff(self, request):
        params = PeriodQuerySerializer(data=request.query_params)
        params.is_valid(raise_exception=True)
        d = params.validated_data

        rows = RA.weekly_by_staff(self._hotel(request), d['start'], d['end'], d.get('department'))
        payload = [{
            'year': r['year'],
            'week': r['week'],
            'staff_id': r['staff_id'],
