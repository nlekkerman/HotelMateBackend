"""
Import new Level 5 questions - scientifically accurate knowledge trap
Usage: python import_new_level5.py
"""
import os
import django
...
print('='*70)
print('LEVEL 5 REPLACEMENT IMPORT')
print('='*70)

# Step 1: Delete old questions
quiz = Quiz.objects.filter(difficulty_level=5).first()
if quiz:
    old_count = quiz.questions.count()
    quiz.questions.all().delete()
    print(f'\n✓ Deleted {old_count} old Level 5 questions')
else:
    print('\n✗ Level 5 quiz not found!')
    exit(1)

# Step 2: Import new questions
imported = 0
for index, q_data in enumerate(new_questions):
...
print(f'✓ Imported {imported} new Level 5 questions')
print(f'✓ Total questions in Level 5: {quiz.questions.count()}')
print('='*70)
