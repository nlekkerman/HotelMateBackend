"""
Script to create GitHub issues for the Booking Phase Backend implementation.
Run this script to automatically create all 5 issues in the GitHub repository.
"""
...
REPO_OWNER = 'nlekkerman'
REPO_NAME = 'HotelMateBackend'
...
- Endpoint is safe to call from frontend booking UI.''',
        'labels': ['backend', 'booking', 'api', 'enhancement']
    },
    {
...
- API is ready to be consumed by booking confirmation screen.''',
        'labels': ['backend', 'booking', 'api', 'pricing', 'enhancement']
    },
    {
...
- Booking is persisted and can be retrieved in later endpoints.''',
        'labels': ['backend', 'booking', 'api', 'database', 'enhancement']
    },
    {
...
- Failed or cancelled payments do not incorrectly confirm bookings.''',
        'labels': ['backend', 'booking', 'payment', 'integration', 'stripe', 'enhancement']
    },
    {
...
- Email logic is idempotent or guarded so the same confirmation is not spammed multiple times by accident.''',
        'labels': ['backend', 'booking', 'email', 'notifications', 'enhancement']
    }
]


def create_github_issues():
    """Create all GitHub issues for the booking phase."""
    ...
    for idx, issue_data in enumerate(ISSUES, 1):
        print(f"\nCreating Issue {idx}: {issue_data['title']}...")
        ...
        try:
            ...
            if response.status_code == 201:
                ...
                print(f"✓ Created successfully: {issue['html_url']}")
            else:
    ...
    print(f"\n{'='*60}")
    print(f"Summary: {len(created_issues)} of {len(ISSUES)} issues created successfully")
    print(f"{'='*60}")
...
if __name__ == '__main__':
    print("="*60)
    print("GitHub Issue Creation - Booking Phase Backend")
    print("="*60)
