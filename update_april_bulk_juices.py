"""
Update April 2025 Bulk Juices counted stock to 20.5 bottles for all items.
"""
import os
import django
...
print("=" * 100)
print("UPDATE APRIL 2025 BULK JUICES - SET TO 20.5 BOTTLES")
print("=" * 100)
print()
...
# Get all Bulk Juices lines (category M, subcategory BULK_JUICES)
bulk_juice_lines = StocktakeLine.objects.filter(
    stocktake=april_stocktake,
    item__category__code='M',
    item__subcategory='BULK_JUICES'
).select_related('item')

total_lines = bulk_juice_lines.count()
print(f"Found {total_lines} Bulk Juices items")
print()

if total_lines == 0:
    print("❌ No bulk juices items found in this stocktake!")
    exit(1)

# Confirm before updating
print("This will set counted stock to:")
print("  - counted_full_units: 20 (bottles)")
print("  - counted_partial_units: 0.5 (half bottle)")
print("  Total: 20.5 bottles")
print()
...
for line in bulk_juice_lines:
    ...
    # Update to 20.5 bottles (20 full + 0.5 partial)
    line.counted_full_units = Decimal('20.00')
...
print("-" * 100)
print(f"✅ Updated {updated} Bulk Juices items")
print()

# Calculate new totals
total_counted_value = sum(line.counted_value for line in bulk_juice_lines)
total_variance_value = sum(line.variance_value for line in bulk_juice_lines)
