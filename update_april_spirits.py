"""
Update April 2025 Spirits counted stock to 3.5 bottles for all items.
"""
import os
import django
...
print("=" * 100)
print("UPDATE APRIL 2025 SPIRITS - SET TO 3.5 BOTTLES")
print("=" * 100)
print()
...
# Get all Spirits lines (category S)
spirits_lines = StocktakeLine.objects.filter(
    stocktake=april_stocktake,
    item__category__code='S'
).select_related('item')

total_lines = spirits_lines.count()
print(f"Found {total_lines} Spirits items")
print()

if total_lines == 0:
    print("❌ No spirits items found in this stocktake!")
    exit(1)
...
for line in spirits_lines:
    ...
    print(f"       {old_counted_qty:.2f} shots (€{old_counted_value:.2f})")
    print(f"  New: {float(line.counted_full_units):.2f} bottles")
    print(f"       {line.counted_qty:.2f} shots "
          f"(€{line.counted_value:.2f})")
    print(f"  Variance: {line.variance_qty:.2f} shots "
          f"(€{line.variance_value:.2f})")
    print()

print("-" * 100)
print(f"✅ Updated {updated} Spirits items")
print()

# Calculate new totals
total_counted_value = sum(line.counted_value for line in spirits_lines)
total_variance_value = sum(line.variance_value for line in spirits_lines)
