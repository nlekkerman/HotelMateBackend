"""
Update April 2025 Soft Drinks counted stock to 2 cases + 8 bottles for all items.
"""
import os
import django
...
print("=" * 100)
print("UPDATE APRIL 2025 SOFT DRINKS - SET TO 2 CASES + 8 BOTTLES")
print("=" * 100)
print()
...
# Get all Soft Drinks lines (category M, subcategory SOFT_DRINKS)
softdrink_lines = StocktakeLine.objects.filter(
    stocktake=april_stocktake,
    item__category__code='M',
    item__subcategory='SOFT_DRINKS'
).select_related('item')

total_lines = softdrink_lines.count()
print(f"Found {total_lines} Soft Drinks items")
print()

if total_lines == 0:
    print("❌ No soft drinks items found in this stocktake!")
    exit(1)
...
for line in softdrink_lines:
    ...
    print(f"  Old: {old_full} cases + {old_partial} bottles")
    print(f"       = {old_counted_qty:.2f} bottles (€{old_counted_value:.2f})")
    print(f"  New: {line.counted_full_units} cases + "
          f"{line.counted_partial_units} bottles")
    print(f"       = {line.counted_qty:.2f} bottles "
...
print("-" * 100)
print(f"✅ Updated {updated} Soft Drinks items")
print()

# Calculate new totals
total_counted_value = sum(line.counted_value for line in softdrink_lines)
total_variance_value = sum(line.variance_value for line in softdrink_lines)
