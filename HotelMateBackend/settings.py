...
try:
    ...
    async def _debug_connect(self):
        builtins.print(
            f"[DEBUG][RedisConn] connect → host={self.host}, port={self.port}, "
            f"ssl_context={getattr(self, 'ssl_context', None)}"
        )
        try:
    ...
    builtins.print(f"[DEBUG] Could not patch RedisConn → {e!r}")


# Patch Channels-Redis group_send
try:
    ...
    builtins.print(f"[DEBUG] Could not patch ChannelLayer → {e!r}")


# Pre-build SSLContext for use below
REDIS_SSL_CONTEXT = ssl.create_default_context(cafile=certifi.where())
builtins.print(f"[DEBUG] Built REDIS_SSL_CONTEXT → verify_mode={REDIS_SSL_CONTEXT.verify_mode}")
